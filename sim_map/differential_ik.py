"""
Code from : https://github.com/achuwilson/pydrake_iiwa/blob/main/differential_ik.py
"""

import numpy as np

from pydrake.common.all import AngleAxis
from pydrake.planning import DoDifferentialInverseKinematics  # FIXME
...
class DifferentialIK(LeafSystem):
    ...
    def __init__(self, robot, frame_E, parameters, time_step):
        ...
        assert not self.robot.GetPositionsAndVelocities(self.robot_context)[
        ...
        self.DeclareInputPort("rpy_xyz_desired", PortDataType.kVectorValued, 6)
    ...
    def ForwardKinematics(self, q):
        ...
        return self.robot.EvalBodyPoseInWorld(self.robot_context, self.frame_E.body())
    ...
    def DoCalcDiscreteVariableUpdates(self, context, events, discrete_state):
        ...
        X_WE_desired = RigidTransform(
            RollPitchYaw(rpy_xyz_desired[:3]), rpy_xyz_desired[-3:]
        ).GetAsIsometry3()
        ...
        result = DoDifferentialInverseKinematics(
            self.robot, self.robot_context, X_WE_desired, self.frame_E, self.parameters
        )
